...
{#if href}
  <a {href} {target} sveltekit:prefetch class="{$$props.class} {form} {size} {color}">
    <slot />
    {#if target === '_blank'}
      <svg
...
{:else}
  <button
    ...
    class="{$$props.class} {form} {size} {color}"
    {type}
...
<style>
  a,
  button {
    @apply inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 hover:text-white uppercase;
  ...
  .black {
    @apply focus:ring-gray-500 text-gray-800  hover:bg-gray-500 border-gray-500;
  }
  .primary {
